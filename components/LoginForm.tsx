	...
	return (
		<Form onSubmit={e => handleSubmit(e)}>
			<FormGroup style={{ marginBottom: '7px' }}>
				<TextField
					error={Boolean(alertMessages.filter(alert => alert.alertType === 'email' || alert.msg === 'Invalid Credentials').length)}
					type='email'
					name='email'
			...
			<FormGroup>
				<TextField
					error={Boolean(alertMessages.filter(alert => alert.alertType === 'password' || alert.msg === 'Invalid Credentials').length)}
					type='password'
					name='password'
